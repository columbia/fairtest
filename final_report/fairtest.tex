...
\begin{figure}[h]
 \includegraphics[width=0.49\textwidth]
  ...
  \caption{\textbf{FairTest Overview}. This figures shows the basic workflow of
  \textit{FairTest}.}
...
is shown in Figure~\ref{fig:FairtestOverview}. The framework records user
attributes (both protected and unprotected inputs) and the appliation output.
Based on the correlation between the output and certain protected private data 
(such as gender, race, etc.), \textit{FairTest} discovers if any
\textit{privacy bugs} can be discovered based on the statistical parity model. 
...
in \textit{FairTest}. Also, to address privacy on the testing framework,
\textit{FairTest} does not require identifiable user information, such as
...
Overall, \textit{FairTest} includes three parts: a RESTful input API that
gathers data from the application tested; a statistical engine that finds
statistical parity between sentitive user data and the price descrimination,
and a report tool that shows the results.
