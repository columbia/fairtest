...
private[python] class PythonMLLibAPI extends Serializable {
  ...
  def trainDecisionTreeBiasDiscovery(
      data: JavaRDD[LabeledPoint],
      algoStr: String,
      numClasses: Int,
      dimOut: Int,
    ...
    val algo = Algo.fromString(algoStr)
    val measure = Measure.fromString(measureStr)
    val aggregator = Aggregator.fromString(aggregatorStr)
    val impurity = Bias((dimOut, dimSens), measure, aggregator, conf)
