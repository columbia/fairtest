...
object DecisionTree extends Serializable with Logging {
  ...
  */
  ...
  def findBiasedClusters(
    ...
    val meas = Measure.fromString(measure)
    val agg = Aggregator.fromString(aggregator)
    val impurity = Bias(dim, meas, agg, conf)
    val strategy = new Strategy(Classification, impurity, maxDepth, numClasses, maxBins,
      Sort, categoricalFeaturesInfo)
    new DecisionTree(strategy).run(input)
  }

  /**
   * Java-friendly API for [[org.apache.spark.mllib.tree.DecisionTree$#findBiasedClusters]]
   */
  def findBiasedClusters(
      input: JavaRDD[LabeledPoint],
      numClasses: Int,
    ...
    findBiasedClusters(input.rdd, numClasses, dim, measure, aggregator, conf,
      categoricalFeaturesInfo.asInstanceOf[java.util.Map[Int, Int]].asScala.toMap,
      maxDepth, maxBins)
  }
  ...
  private def calculateGainForSplit(
    ...
    val gain = metadata.impurity match {
        case bias @ Bias(_, _, _, _) => bias.calcuate_gain(impurity, leftImpurity, rightImpurity, leftWeight, rightWeight)
        case _ => impurity - leftWeight * leftImpurity - rightWeight * rightImpurity
    }
