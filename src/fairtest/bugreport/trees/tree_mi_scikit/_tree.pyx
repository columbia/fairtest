...
from ._utils cimport PriorityHeap, PriorityHeapRecord
...
MODE_AVG = 0
MODE_MAX = 1
cdef SIZE_t _MODE_AVG = MODE_AVG
cdef SIZE_t _MODE_MAX = MODE_MAX

cdef class Mutual_Info(ClassificationCriterion):
    # cardinality of sensitive and output attributes
    cdef SIZE_t num_SENS
    cdef SIZE_t num_OUT   
    cdef SIZE_t mode 
    
    # initialize attribute ranges
    def set_params(self, num_sens, num_out, mode=MODE_AVG):
    ...
    cdef double impurity_improvement(self, double impurity) nogil:
        #
        # Compute the conditional mutual information as an impurity
        # improvement score.
        # 
        cdef double impurity_left
        cdef double impurity_right
        ...
        if self.mode == _MODE_AVG:
            return self.weighted_n_right / self.weighted_n_node_samples * impurity_right + self.weighted_n_left / self.weighted_n_node_samples * impurity_left
        else:
            return max(impurity_right, impurity_left)
    ...
    cdef double node_impurity(self) nogil:
        ...
        with gil:
            # count all the outputs of the form (S,O)
            ct = np.zeros(self.n_classes[0])
            for c in range(n_classes[0]):
                ct[c] = label_count_total[c]
            
            # build a S x O contingency table
            ct = ct.reshape([self.num_SENS,self.num_OUT])
            
            # compute the mutual information
            mi = mutual_info_score(None, None, contingency=ct)
            return mi
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        with gil:
            ...
            # count all the outputs of the form (S,O) for both children
            ct_left = np.zeros(self.n_classes[0])
            ct_right = np.zeros(self.n_classes[0])
            for c in range(n_classes[0]):
                ct_left[c] = label_count_left[c]
                ct_right[c] = label_count_right[c]
            
            # build a S x O contingency table for each child
            ct_left = ct_left.reshape([self.num_SENS,self.num_OUT])
            ct_right = ct_right.reshape([self.num_SENS,self.num_OUT])
            
            # compute the mutual information scores
            mi_left = mutual_info_score(None, None, contingency=ct_left)
            mi_right = mutual_info_score(None, None, contingency=ct_right)

            impurity_left[0] = mi_left
            impurity_right[0] = mi_right
    ...
    cdef void node_split(self, double impurity, SplitRecord* split,
        ...
        while (f_i > n_total_constants and  # Stop early if remaining features
            ...
            if f_j < n_known_constants:
                ...
                #with gil:
                #    print 'testing feature {}'.format(current.feature)
