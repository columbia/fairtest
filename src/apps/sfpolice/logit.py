...
def main(argv):
    ...
    predict_probas = model.predict_proba(features_test_original.as_matrix())
    predictions = model.predict(features_test_original.as_matrix())
    entropy = map(lambda x: sp.stats.entropy(x), predict_probas)
